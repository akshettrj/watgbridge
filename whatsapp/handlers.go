...
func bridgeMessageBody(base, forwardSuffix string) string {
	if forwardSuffix == "" {
		return base
	}
	return base + forwardSuffix
}
...
func MessageFromOthersEventHandler(text string, v *events.Message, isEdited bool) {
	...
	statusChat := v.Info.Chat.ToNonAD().String() == "status@broadcast"
	// Groups: 👤 name (+phone); private 1:1 and non-status broadcast → topic title only.
	if v.Info.IsGroup && !statusChat {
		bridgedText += utils.WaSenderHTMLBlock(v.Info.MessageSource.Sender.ToNonAD()) + "\n\n"
	}
	...
	if isEdited {
		...
		if contextInfo != nil {
			...
			if contextInfo.GetIsForwarded() {
				forwardSuffix = fmt.Sprintf("\n⏩: Forwarded %v times", contextInfo.GetForwardingScore())
			}
				...
				for _, jid := range mentioned {
					...
					if parsedJid.User == waClient.Store.ID.User {
						...
						if err != nil {
							...
							_, _ = tgSendBridgeTargetMessage(tgBot, cfg.Telegram.TargetChatID, tagInfoText, &gotgbot.SendMessageOpts{
	...
	if bridgedText != "" && !strings.HasSuffix(bridgedText, "\n\n") {
		bridgedText += "\n"
	}
	...
	if !threadIdFound {
		...
		if statusChat {
			...
			if (v.Info.MessageSource.AddressingMode == waTypes.AddressingModePN) || v.Info.MessageSource.SenderAlt.IsEmpty() {
				threadId, _, err = utils.TgGetOrMakeThreadFromWa(v.Info.MessageSource.Sender.ToNonAD(), cfg.Telegram.TargetChatID, "")
			} else {
				threadId, _, err = utils.TgGetOrMakeThreadFromWa(v.Info.MessageSource.SenderAlt.ToNonAD(), cfg.Telegram.TargetChatID, "")
			}
			if err != nil {
			...
			threadId, _, err = utils.TgGetOrMakeThreadFromWa(v.Info.Chat, cfg.Telegram.TargetChatID,
			...
			threadId, _, err = utils.TgGetOrMakeThreadFromWa(target_chat_jid, cfg.Telegram.TargetChatID, "")
			if err != nil {
				utils.TgSendErrorById(tgBot, cfg.Telegram.TargetChatID, 0, fmt.Sprintf("failed to create/find thread id for '%s'",
	...
	if isEdited && replyToMsgId != 0 {
		_, _ = tgSendBridgeTargetMessage(tgBot, cfg.Telegram.TargetChatID, "<b>Edited</b>", &gotgbot.SendMessageOpts{
	...
	if v.Message.GetImageMessage() != nil {
		...
		if cfg.WhatsApp.SkipImages {
			...
			sentMsg, _ := tgSendBridgeTargetMessage(tgBot, cfg.Telegram.TargetChatID, bridgeMessageBody(bridgedText, forwardSuffix), &gotgbot.SendMessageOpts{
				ReplyParameters: &gotgbot.ReplyParameters{
					MessageId: replyToMsgId,
				},
				MessageThreadId: threadId,
			...
			sentMsg, _ := tgBot.SendPhoto(cfg.Telegram.TargetChatID, &gotgbot.FileReader{Data: bytes.NewReader(imageBytes)}, &gotgbot.SendPhotoOpts{
				Caption: bridgeMessageBody(bridgedText, forwardSuffix),
				ReplyParameters: &gotgbot.ReplyParameters{
					MessageId: replyToMsgId,
				},
		...
		for _, contactMsg := range contactsMsg.Contacts {
			...
			card, err := decoder.Decode()
			if err != nil {
				_, _ = tgSendBridgeTargetMessage(tgBot, cfg.Telegram.TargetChatID, "Couldn't send the vCard as failed to parse it",
		...
		sentMsg, err := tgSendBridgeTargetMessage(tgBot, cfg.Telegram.TargetChatID, bridgeMessageBody(bridgedText, forwardSuffix), &gotgbot.SendMessageOpts{
			ReplyParameters: &gotgbot.ReplyParameters{
				MessageId: replyToMsgId,
			},
			MessageThreadId: threadId,
...
func UndecryptableMessageEventHandler(v *events.UndecryptableMessage) {
	...
	sentMsg, err := tgSendBridgeTargetMessage(tgBot, cfg.Telegram.TargetChatID, bridgedText, &gotgbot.SendMessageOpts{
		MessageThreadId: threadId,
	})
	if err != nil {
...
func CallOfferEventHandler(v *events.CallOffer) {
	...
	if err := utils.TgSendTextById(tgBot, cfg.Telegram.TargetChatID, callThreadId, bridgeText); err != nil {
		utils.TgSendErrorById(tgBot, cfg.Telegram.TargetChatID, 0, "Failed to send call notification to Calls topic", err)
	}

	// Same notification in the contact's forum topic (not only global Calls).
	callerJID := v.CallCreator.ToNonAD()
	contactThreadId, _, err := utils.TgGetOrMakeThreadFromWa(callerJID, cfg.Telegram.TargetChatID, "")
	if err != nil {
		utils.TgSendErrorById(tgBot, cfg.Telegram.TargetChatID, 0, fmt.Sprintf("Failed to resolve thread for call from %s", html.EscapeString(callerJID.String())), err)
		return
	}
	if contactThreadId != callThreadId {
		_ = utils.TgSendTextById(tgBot, cfg.Telegram.TargetChatID, contactThreadId, bridgeText)
	}
}
...
func RevokedMessageEventHandler(v *events.Message) {
	...
	_, _ = tgSendBridgeTargetMessage(tgBot, tgChatId, "<b>Deleted</b>", &gotgbot.SendMessageOpts{
...
func PictureEventHandler(v *events.Picture) {
	...
	if v.JID.Server == waTypes.GroupServer {
		tgThreadId, _, err = utils.TgGetOrMakeThreadFromWa(v.JID.ToNonAD(), cfg.Telegram.TargetChatID, utils.WaGetGroupName(v.JID))
		if err != nil {
			logger.Warn(
